...
namespace SettlersOfCatan
{
	public partial class GameScreen : Form
	{
		...
		private void setupWaterHexes()
		{
			...
			// in order: x, x_diff, y, y_diff
			int[] topCoords = new int[4] { 225, 150, 0, 825 };
			int[] leftCoords = new int[4] { 150, 75, 75, 600 };
			int[] rightCoords = new int[4] { 750, 75, 75, 600 };

			// Set up top and bottom (short hexes)
			for (int i = 0; i < 4; i++)
			{
				addPictureBox(waterCount, topCoords, true, false);
				waterCount++;

				addPictureBox(waterCount, topCoords, true, true);
				waterCount++;
			...
			for (int i = 0; i < 3; i++)
			{
				addPictureBox(waterCount, leftCoords, false, false);
				waterCount++;

				if (i == 2) break;

				addPictureBox(waterCount, leftCoords, false, true);
				waterCount++;

				leftCoords[0] -= leftCoords[1];
				leftCoords[2] += 150;
				leftCoords[3] -= 150 * (i + 2);
				// x -= x_diff
				// y += 150
				// y_diff -= 150*(i + 2)
			}
			...
			for (int i = 0; i < 3; i++)
			{
				addPictureBox(waterCount, rightCoords, false, false);
				waterCount++;

				if (i == 2) break;

				addPictureBox(waterCount, rightCoords, false, true);
				waterCount++;

				rightCoords[0] += rightCoords[1];
				rightCoords[2] += 150;
				rightCoords[3] -= 150*(i+2);
				//x += x_diff;
				//y += 150;
				//y_diff -= 150*(i + 2);
			}
